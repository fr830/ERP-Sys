...
/// </summary>
public class LoginFunctionView
{
    public String sql = "";
    ...
    public bool checkUserStatus(String username, String password)
    {
        ...
        System.IO.File.AppendAllText("D:\\Test.log", MD5Hash(password) + username);
        var result = db.QueryValue(sql, MD5Hash(password), username);
        if (result == "Active")
        {
            userActive = true;
    ...
    {
        var sql = "SELECT TOP 1 employee_id FROM AccountInfoes WHERE username = @0";
        Database db = Database.Open("InvoiceSysDBContext");
        String result = db.QueryValue(sql, username).ToString();
        return result;
    }
